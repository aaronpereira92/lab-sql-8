...
/*
Which tables are needed: 4 tables: store, address, city, country
Do they have a common column: store-address(address_id), address-city(city_id), city-country(country_id)
...
/*
Which tables are needed: 3 tables: store, staff, payment
Do they have a common column: store-staff(store_id), staff-payment(staff_id)
...
-- Q7 & Q8 skipped as we did not learn about SELF JOIN yet
...
SELECT f.film_id, f.title,  count(fa.actor_id) over(partition by a.actor_id) as total_films_acted, concat(a.first_name," ",a.last_name) as actor_name FROM sakila.film AS f
